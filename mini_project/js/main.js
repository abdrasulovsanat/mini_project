...
function updatePost() {
    ...
    if(!checkOwnerPost(postId)){
        alert('There is no post with this id or you are not author of such a post');
        return;
    };
...
function addComment() {
    if(!inSystem){
        alert('Only authorized users can create comments posts!');
        return;
    };
    let postId = +prompt('Enter post id');
    let postObj = getPostObj(postId);
...
//   updArr.push(arr[l]);
// }
// updArr = arr.sort((a, b) => b.length - a.length);
// console.log(updArr[0]);
